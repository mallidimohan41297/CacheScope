> **Engineering derivative:** This repository is based on the upstream project documented in `DERIVATION.md`.
> The original license and attribution are retained. The additions in this distribution are marked in the
> engineering documentation rather than presented as an original reimplementation.
...
This distribution adds `experiments/workload_generator.py` and
`scripts/compare_policies.sh` for deterministic workload experiments.
See `experiments/README.md` for reproducibility requirements.

Build and compare:

```bash
make
python3 experiments/workload_generator.py --requests 100000 --objects 10000 --seed 7
./scripts/compare_policies.sh experiments/workload.trace 1048576
```
...
python3 experiments/workload_generator.py --requests 5000 --objects 100 --seed 42 --output /tmp/cache.trace
./scripts/compare_policies.sh /tmp/cache.trace 65536
```

The policy tests cover deterministic LRU/FIFO eviction behavior and accounting
invariants for GDS, GDSF, LFUDA, and LRUK. The experiment command is seeded so
